...
import knowledge.project.baseclass.XMLTagger;
...
public class XMLLightTagger extends XMLTagger{
	...
	private StringBuffer toTagNodeLightVersion(Element rootElement, Node node) {
	...
	//
	private void tagXMLFileLightVersion(String xmlFilePath, String outXMLFilePath) {

		Document document = buildDocument(xmlFilePath);		
		if(document == null) {
		...
		if(rootElement == null) {
			...
			for(int i = 0; i < nodeListLength; i++) {
				...
				if(this.tobeTaggedList.contains(nodeName)) {				//nodes to be parsed
					tagPlusContent = this.toTagNodeLightVersion(rootElement, subNode);
				} else {												//nodes not to be parsed
					tagPlusContent = this.toReserveLightVersionNode(rootElement, subNode);
				}
	...
	//tag the XML foler
	public void tagXMLFolder_LightVersion(String inFolderPath, String outFolderPath_FullVersion) {
		...
		for(File xmlFile : xmlFileList) {
			...
			tagXMLFileLightVersion(xmlFilePath, outXMLFilePath);
		}//for
	...
	public static void main(String[] args) {
		...
		XMLLightTagger xmlTagger_lightversion = new XMLLightTagger();
		xmlTagger_lightversion.tagXMLFolder_LightVersion(inFolderPath, outFolderPath_FullVersion);
