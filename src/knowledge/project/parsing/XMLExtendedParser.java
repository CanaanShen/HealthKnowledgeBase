...
public class XMLExtendedParser extends XMLParser{
	...
	private StringBuffer generateUnChangedNode(Node node) {
		...
		if(!nodeName.equals("ingredient")) {
			...
			if(mark == 0) {						//no content in the ingredient tag
	...
	private StringBuffer generateNewNode(Node node) {
		NodeList subNodeList = node.getChildNodes();
		int mark = 0;
		for(int i = 0; i < subNodeList.getLength(); i++) {			//already be parsed
			Node subNode = subNodeList.item(i);
		...
		if(mark == 1) {										//a old node
			return keepOldNode(node);
		} else {											//a new node
	...
	//keep an old node that has been parsed
	private StringBuffer keepOldNode(Node node) {
		StringBuffer tagPlusContent = new StringBuffer("");
		String nodeName = node.getNodeName();
		...
		for(int i = 0; i < subNodeListLength; i++) {
			...
			tagPlusContent.append(ConfigUtil.Second_Indent + "<" + subNodeName + ">\n");
			String formatedText = CommonUtil.formatedText(subText, ConfigUtil.Third_Indent);
	...
	private StringBuffer generateNewNode(String nodeName, String text) {
		...
		if(text == null || text.length() == 0 || textTemp == null || textTemp.length() == 0) {
			tagPlusContent.append("\n");
		} else {
...
//			System.out.println("pos begins");
	...
	// parse xml files
	private void parseExtendedXMLFolder(String inFolderPath, String outFolderPath) {
	...
	public static void main(String[] args) {
		...
		XMLExtendedParser xmlExtendedParser = new XMLExtendedParser();
		xmlExtendedParser.parseExtendedXMLFolder(inFolderPath, outFolderPath);
